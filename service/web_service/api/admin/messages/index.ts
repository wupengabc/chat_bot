...
type MessagePosition = "chat" | "system";
...
interface BangxiStorage {
    search_admin_messages(page: number, limit: number, filters: MessageFilters): MessageSearchResult;
    get_message_context(id: number, page: number | undefined, limit: number, filters: MessageFilters): {messages: MessageSearchResult["messages"]; total: number; page: number; limit: number; total_pages: number} | null;
}

const messagePositions = new Set(["chat", "system"]);
const messageTypes = new Set(["public", "private"]);
...
export async function init(app: Express) {
    app.get("/api/admin/messages/:id/context", require_admin, (request: AdminRequest, response: Response) => {
        ...
        const position = parseEnumFilter<MessagePosition>(request.query.position, messagePositions);
        const messageType = parseEnumFilter<MessageType>(request.query.message_type, messageTypes);
        if (id === null || page === null || requestedLimit === null || username === null || createTime === null || createTimeFrom === null || createTimeTo === null || content === null || position === null || messageType === null) {
            response.status(400).json({success: false, message: "上下文查询参数无效"});
            return;
        }
        const canViewPrivate = request.admin?.role === "owner";
        if (!canViewPrivate && messageType === "private") {
            response.status(403).json({success: false, message: "private 消息仅 owner 可查看"});
            return;
        }
        ...
        const result = storage.get_message_context(id, page, Math.min(requestedLimit, 100), {username, create_time: createTime, create_time_from: createTimeFrom ? new Date(fromTimestamp!).toISOString() : undefined, create_time_to: createTimeTo ? new Date(toTimestamp!).toISOString() : undefined, content, position, message_type: messageType, include_private: canViewPrivate});
        if (!result) {
    ...
    app.get("/api/admin/messages", require_admin, (request: AdminRequest, response: Response) => {
        ...
        if (position === null) {
            response.status(400).json({success: false, message: "position 只能是 chat 或 system"});
            return;
        }
        if (messageType === null) {
            response.status(400).json({success: false, message: "message_type 只能是 public 或 private"});
            return;
        }
        const canViewPrivate = request.admin?.role === "owner";
        if (!canViewPrivate && messageType === "private") {
        ...
        const storage = get_storage("bangxi_server_storage") as BangxiStorage | undefined;
        if (!storage?.search_admin_messages) {
            response.status(503).json({success: false, message: "bangxi_server_storage 不可用"});
            return;
        }

        const result = storage.search_admin_messages(page, limit, {
            username,
            ...
            position,
            message_type: messageType,
            include_private: canViewPrivate,
        });
