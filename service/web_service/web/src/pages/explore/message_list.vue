...
import {messageContextRequest, messagesRequest, playerAvatarUrl, playerNamesRequest, type PublicMessage} from '../../utils/login'

type PageLimit = '10' | '20' | '50'

const pageLimitOptions: PageLimit[] = ['10', '20', '50']
...
async function loadMessages(nextPage = 1) {
  ...
  error.value = ''
  try {
    const result = await messagesRequest(nextPage, Number(pageLimit.value), {
...
async function loadMessageContext(messageId: number, nextPage?: number) {
  ...
  contextError.value = ''
  try {
    const result = await messageContextRequest(messageId, nextPage, {username: contextUsername.value, create_time_from: contextCreateTime.value.from, create_time_to: contextCreateTime.value.to, content: contextContent.value})
...
<template>
  <section class="message-panel">
    <div class="message-toolbar">
      <div class="message-summary"><span class="status-dot" :class="{ muted: !loaded && !loading }" /><div><strong>公共消息</strong><small v-if="loading" class="syncing-label">正在获取<span aria-hidden="true"><i/><i/><i/></span></small><small v-else>{{ total.toLocaleString() }}</small></div></div>
      <form class="message-filters" @submit.prevent="searchMessages">
        ...
        <div class="message-filter-actions">
          ...
          <button class="message-clear-button" type="button" :disabled="!username && !content && !createTime.from && !createTime.to" @click="clearFilters">清除</button>
        </div>
      </form>
      ...
      <Transition name="message-page" mode="out-in">
        <div :key="page" class="message-page">
          <div v-if="loading && !messages.length" class="explore-empty message-loading">
            ...
            <strong>正在读取公共消息</strong><span>从邦溪世界档案同步最新聊天记录</span>
          </div>
          ...
          <div v-else-if="!messages.length" class="explore-empty message-unavailable">
            <span class="empty-code">NO MATCH</span><strong>没有匹配的公开消息</strong><span>调整筛选条件后重新查询。</span>
          </div>
          <div v-else class="message-stream">
            <article v-for="message in messages" :key="message.id" class="message-item">
              ...
              <div class="message-body">
                <header class="message-head"><strong>{{ message.username }}</strong></header>
      ...
      <div class="context-dialog">
        <form class="context-filters" @submit.prevent="contextCenterId && loadMessageContext(contextCenterId, 1)">
          ...
          <div class="context-filter-actions">
            ...
            <button class="context-clear-button" type="button" :disabled="!contextUsername && !contextContent && !contextCreateTime.from && !contextCreateTime.to" @click="contextUsername=''; contextContent=''; contextCreateTime={from:'',to:''}; contextCenterId && loadMessageContext(contextCenterId, 1)">清除</button>
          </div>
        </form>
        ...
        <div v-else class="context-stream">
          <article v-for="message in contextMessages" :key="message.id" class="context-item" :class="{ focused: message.id === contextCenterId }"><time>{{ formatCopyTime(message.create_time) }}</time><strong>{{ message.username }}</strong><p>{{ messageContent(message.content) || '（空消息）' }}</p></article>
        </div>
...
<style scoped>
.message-panel { display: flex; flex-direction: column; flex: 1 1 auto; min-height: 0; margin-top: 10px; overflow: visible; color: var(--panel-text); background: var(--panel-bg); border: 1px solid var(--border); border-radius: 5px; box-shadow: 0 18px 46px var(--shadow); }
.message-toolbar { position: relative; z-index: 7; display: grid; grid-template-columns: auto minmax(0, 1fr) 34px; align-items: center; gap: 8px; flex: 0 0 48px; height: 48px; min-height: 48px; box-sizing: border-box; padding: 6px 10px; background: var(--panel-bg); border-bottom: 1px solid var(--border); }
...
.message-filters { display: grid; grid-template-columns: minmax(90px, .6fr) minmax(140px, .9fr) minmax(260px, 1.35fr) auto; gap: 6px; align-items: center; min-width: 0; }
...
.message-filters input { width: 100%; height: 34px; padding: 0 9px; color: var(--panel-text); background: var(--surface); border: 1px solid var(--border); border-radius: 4px; outline: none; font-size: 11px; }
...
.player-filter { position: relative; display: block !important; }
.player-filter input { height: 34px; padding-right: 38px; border-radius: 4px; }
.player-filter :deep(.tooltip-trigger) { position: absolute; top: 0; right: 0; display: grid; width: 34px; min-width: 34px; height: 34px; }.player-filter > :deep(.tooltip-trigger) > button { display: grid; place-items: center; width: 34px; height: 34px; padding: 0; color: var(--panel-text); background: transparent; border: 0; border-left: 1px solid var(--border); border-radius: 0 4px 4px 0; font-size: 15px; }
...
.message-head { display: flex; align-items: baseline; }
...
.context-item { display: grid; grid-template-columns: 118px 110px minmax(0, 1fr); gap: 8px; align-items: baseline; padding: 8px 10px; border-bottom: 1px solid var(--border); }
...
.context-item strong { color: var(--accent); font-size: 10px; overflow-wrap: anywhere; }
...
@media (max-width: 1040px) { .message-toolbar { grid-template-columns: auto minmax(0, 1fr) 34px; } .message-summary div { display: none; } .message-summary { padding-right: 8px; } .message-filters { grid-template-columns: .65fr 1fr 1.4fr auto; } }
@media (max-width: 700px) {
  .message-toolbar { grid-template-columns: minmax(0, 1fr) 34px; align-items: start; padding: 8px; }
  .message-summary { display: none; }
  .message-filters { grid-column: 1; grid-row: 1; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); }
  .range-filter, .message-filter-actions { grid-column: span 2; }
  .message-filter-actions > * { flex: 1; }
  .refresh-button { grid-column: 2; grid-row: 1; }
}
@media (max-width: 480px) {
  .message-filters { grid-template-columns: minmax(0, 1fr); }
  .range-filter, .message-filter-actions { grid-column: auto; }
  .message-scroll { padding-inline: 8px; }
  .message-pagination { padding-inline: 8px; }
  .message-pagination > span { display: none; }
  .page-controls { width: 100%; justify-content: center; }
  .page-controls label span { display: none; }
  .message-pagination .edge-page { padding-inline: 6px; }
  .message-item { grid-template-columns: 28px minmax(0, 1fr) 28px; align-items: start; }
  .message-body { grid-column: 2; }
  .message-meta { grid-column: 2; grid-row: 2; display: flex; justify-items: start; gap: 7px; min-width: 0; text-align: left; }
  .message-meta span { padding-left: 7px; border-left: 1px solid var(--border); }
   .message-item > :deep(.tooltip-trigger) { grid-column: 3; } .message-item > :deep(.tooltip-trigger):nth-last-child(2) { grid-row: 1; } .message-item > :deep(.tooltip-trigger):last-child { grid-row: 2; } .context-item { grid-template-columns: 1fr; gap: 3px; }
}
...
.message-summary, .message-filters, .message-toolbar > .refresh-button { align-self: center; }
.message-summary strong, .message-summary small { line-height: 1.1; }
.message-filters input, .player-filter input,
.player-filter :deep(.tooltip-trigger),
.player-filter > :deep(.tooltip-trigger) > button,
.refresh-button, .message-search-button, .message-clear-button { height: 30px; }
.message-filter-actions button { padding-inline: 8px; }
.message-filters { gap: 5px; }
.range-filter :deep(.range-trigger) { height: 30px; padding-inline: 7px; font-size: 9px; }
.message-summary { gap: 7px; padding-right: 8px; }
.message-summary > div { width: 96px; }
.status-dot { width: 6px; height: 6px; box-shadow: 0 0 0 3px color-mix(in srgb, var(--success) 18%, transparent); }
.status-dot.muted { box-shadow: 0 0 0 3px color-mix(in srgb, var(--muted-text) 16%, transparent); }
</style>
